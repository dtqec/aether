...
(defmacro sync-rpc (message
                    (result-place-or-list destination &key returned?)
                    &body body)
         ...
         (sync-receive (,listen-channel ,message-place)
           ...
           (message-rpc-done
            ,(etypecase result-place-or-list
               (symbol
                `(let ((,result-place-or-list (message-rpc-done-result ,message-place)))
                   ,@decls
                   (declare (ignorable ,result-place-or-list))
                   (unregister ,listen-channel)
                   ,@body))
               (list
                `(destructuring-bind ,result-place-or-list (message-rpc-done-result ,message-place)
                   ,@decls
                   (unregister ,listen-channel)
                   ,@body)))))))))
